// LAB-11 · solução de referência · prepara a Aula 20
...
void carregar_em(std::string& alvo, const std::string& nome) {
  ...
  std::string novo = std::get<std::string>(r);   // pode lançar: ainda não tocamos no alvo
  alvo.swap(novo);                                // não lança: a troca é atômica aqui
}
